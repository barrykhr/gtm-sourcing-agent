 ...
 * screen underneath. /reset-password is the same bare canvas as /login
 * for the same reason — a "forgot password" email link lands someone
 * who isn't logged in yet either. /share/[token] (Batch B) is the one
 * route meant for someone outside the recruiting team — a client with
 * a link, not an account — so it gets the same bare canvas and skips
 * AuthGate entirely rather than bouncing them to /login. */
export function AppShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();

  if (pathname === "/login" || pathname === "/reset-password" || pathname.startsWith("/share/")) {
