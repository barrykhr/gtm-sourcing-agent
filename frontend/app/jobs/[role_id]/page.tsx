  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="eyebrow mb-1.5">Role workspace</p>
                      ...
                      <tr>
                        <td colSpan={7} className="border-t border-zinc-100 bg-zinc-50/50 px-4 py-4 dark:border-zinc-800 dark:bg-zinc-950/50">
                          <div className="flex flex-col gap-3">
                            ...
                            <CommunicationsCard roleId={roleId} candidate={c} run={run} busy={busy} />
...
function CommunicationsCard({
  roleId, candidate, run, busy,
}: {
  roleId: string; candidate: Candidate; run: (name: string, action: () => Promise<unknown>) => void; busy: string | null;
}) {
  const candidateId = candidate.candidate_id;
  const [phoneDraft, setPhoneDraft] = useState(candidate.phone);
  const [prevPhone, setPrevPhone] = useState(candidate.phone);
  const [emailDraft, setEmailDraft] = useState(candidate.email);
  const [prevEmail, setPrevEmail] = useState(candidate.email);
  if (candidate.phone !== prevPhone) { setPrevPhone(candidate.phone); setPhoneDraft(candidate.phone); }
  if (candidate.email !== prevEmail) { setPrevEmail(candidate.email); setEmailDraft(candidate.email); }
  const contactDirty = phoneDraft !== candidate.phone || emailDraft !== candidate.email;
  ...
  const [waMessage, setWaMessage] = useState(
    `Hi ${candidate.name.split(" ")[0] || ""}, reaching out about an opportunity we think could be a strong fit — do you have a few minutes to connect?`,
  );
  ...
  function confirmWhatsApp() {
    ...
    run(`comm-${candidateId}`, () => logAndRefresh("whatsapp", waMessage, candidate.phone));
  }

  function confirmCall() {
    window.location.href = `tel:${candidate.phone}`;
    setConfirmChannel(null);
    run(`comm-${candidateId}`, () =>
      logAndRefresh("call", `Call initiated to ${candidate.phone} (device handoff — no telephony backend, so connection isn't confirmed here).`, candidate.phone),
    );
  }
  ...
  function saveManualEntry() {
    ...
    setManualContent("");
    setManualTranscript("");
    run(`comm-${candidateId}`, () => logAndRefresh(manualChannel, content, "", transcript));
  }
    ...
    <Card title="Conversation history">
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-end gap-2">
          ...
          {contactDirty && (
            <button
              onClick={() => run(`contact-${candidateId}`, () => setCandidateContact(roleId, candidateId, phoneDraft, emailDraft))}
              disabled={busy === `contact-${candidateId}`}
              className="rounded-md bg-indigo-700 px-2.5 py-1 text-xs font-medium text-white hover:bg-indigo-800 disabled:opacity-50"
            >
              {busy === `contact-${candidateId}` ? "Saving…" : "Save contact"}
            </button>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            title={candidate.phone ? "" : "Add a phone number first"}
        ...
        {confirmChannel === "whatsapp" && (
          <ConfirmDialog
            ...
            body={`This opens WhatsApp to message ${candidate.phone}.`}
        ...
        {confirmChannel === "call" && (
          <ConfirmDialog
            ...
            body={`This opens your device's dialer for ${candidate.phone}. Cancel if you'd rather not.`}
            confirmLabel={`Call ${candidate.phone}`}
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            disabled={!manualContent.trim() || busy === `comm-${candidateId}`}
          ...
          >
            {busy === `comm-${candidateId}` ? "Logging…" : "Log entry"}
          </button>
        </div>
